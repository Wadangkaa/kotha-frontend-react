...
const AdminKothaDetailPage = () => {
	...
	const postResponse = async (response: number) => {
		try {
			await apiFetch(`api/admin/kotha/${id}/response`, { method: 'POST', body: JSON.stringify({ response }) });
	...
	return (
		<div>
			<div className="tw-flex tw-gap-4 tw-justify-end tw-mb-4">
				<button onClick={ () => postResponse(1) } className="tw-mt-4 tw-px-4 tw-py-2 tw-bg-green-500 tw-text-white tw-rounded" style={ { maxInlineSize: 'fit-content' } }>
					Approve
				</button>
				<button onClick={ () => postResponse(0) } className="tw-mt-4 tw-px-4 tw-py-2 tw-bg-red-500 tw-text-white tw-rounded" style={ { maxInlineSize: 'fit-content' } }>
					Reject
				</button>
			</div>
...
export default AdminKothaDetailPage;
